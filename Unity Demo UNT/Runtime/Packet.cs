...
namespace Unt.Demo.Runtime
{
    public class Packet
    {
        ...
        public static Packet New(ushort dataId, ushort maxSizeData = 1280) => new Packet(dataId, maxSizeData);
        private Packet(ushort dataId, ushort maxSizeData)
        {
            Data = new byte[maxSizeData];
            AddByte(0);
            AddUShort((ushort)dataId);
        }

        public static Packet New(Header header, ushort maxSizeData = 32) => new Packet(header, maxSizeData);
        private Packet(Header header, ushort maxSizeData)
        {
            Data = new byte[maxSizeData];
            AddByte((byte)header);
        }
